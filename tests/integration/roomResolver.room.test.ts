...
describe('roomResolver.Room', () => {
  ...
  context('1 student, 1 xapi "score" event; result is cached', () => {
    ...
    before(async () => {
      ...
      // cached room
      const existingRoom = await new RoomBuilder()
        .withRoomId(roomId)
        .withAttendanceCount(2)
        .build()
      const existingUserContentScore = await new UserContentScoreBuilder()
        .withroomId(roomId)
        ...
        .withSeen(true)
        .build()
      const existingAnswer = await new AnswerBuilder(existingUserContentScore)
        .withScore(score)
