    ...
    context('1 content requested which exists; not cached', () => {
      it('returns matching content; calls cmsContentApi; result saved in cache', async () => {
        ...
        const authenticationToken =
          endUser.token ?? throwExpression('authentication token is undefined')
        cmsContentApi
          .getLessonMaterialsWithSourceId(h5pId, authenticationToken)
          .resolves([contentDto])

        // Act
        const result = await sut.getLessonMaterialsWithSourceId(
          h5pId,
          authenticationToken,
        )
        ...
        // Assert
        expect(result).to.deep.equal([contentDto])
        cmsContentApi.received(1).getLessonMaterialsWithSourceId(Arg.all())
        const cached = await cache.get(getLessonMaterialsWithSourceIdKey(h5pId))
        expect(cached).to.deep.equal(JSON.stringify([contentDto]))
      })
    })
    ...
    context('1 content requested which exists; result is cached', () => {
      it('returns matching content; does not call cmsContentApi; result still saved in cache', async () => {
        ...
        const authenticationToken =
          endUser.token ?? throwExpression('authentication token is undefined')
        cmsContentApi
          .getLessonMaterialsWithSourceId(contentId, authenticationToken)
          .resolves([contentDto])
        await cache.set(
          getLessonMaterialsWithSourceIdKey(contentId),
          JSON.stringify([contentDto]),
          60,
        )
        ...
        // Assert
        expect(result).to.deep.equal([contentDto])
        cmsContentApi.received(0).getLessonMaterialsWithSourceId(Arg.all())
